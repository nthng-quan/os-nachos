...
Kernel::Kernel(int argc, char **argv)
{
	...
	for (int i = 1; i < argc; i++) {
		if (strcmp(argv[i], "-rs") == 0) {
			ASSERT(i + 1 < argc);
			RandomInit(
				atoi(argv[i + 1])); // initialize pseudo-random
				// number generator
			randomSlice = TRUE;
			i++;
			...
			cout << "Partial usage: nachos [-ci consoleIn] [-co "
				"consoleOut]\n";
#ifndef FILESYS_STUB
...
void Kernel::Initialize()
{
	...
	synchConsoleOut =
		new SynchConsoleOutput(consoleOut); // output to stdout
...
void Kernel::NetworkTest()
{
	if (hostName == 0 || hostName == 1) {
		...
		cout << "Got: " << buffer << " : from " << inPktHdr.from
		     << ", box " << inMailHdr.from << "\n";
		cout.flush();

		// Send acknowledgement to the other machine (using "reply to"
		// mailbox in the message that just arrived
		outPktHdr.to = inPktHdr.from;
		outMailHdr.to = inMailHdr.from;
		outMailHdr.length = strlen(ack) + 1;
		postOfficeOut->Send(outPktHdr, outMailHdr, ack);

		// Wait for the ack from the other machine to the first message
		// we sent
