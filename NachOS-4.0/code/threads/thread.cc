...
void Thread::Sleep(bool finishing)
{
	...
	while ((nextThread = kernel->scheduler->FindNextToRun()) == NULL)
		kernel->interrupt
			->Idle(); // no one to run, wait for an interrupt
...
void Thread::StackAllocate(VoidFunctionPtr func, void *arg)
{
	...
	// everyone else works the other way: from high addresses to low
	// addresses
	...
	*stack = STACK_FENCEPOST;
	...
	stackTop =
		stack + StackSize - 16; // RS6000 requires 64-byte frame marker
	*stack = STACK_FENCEPOST;
#endif
...
#ifdef x86
	// the x86 passes the return address on the stack.  In order for
